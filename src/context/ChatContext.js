...
export const ChatProvider = ({ children }) => {
  ...
  const [messages, setMessages] = useState([]);
  const [activeChatId, setActiveChatId] = useState(null);
  const [activeMode, setActiveMode] = useState("map"); // Default mode
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const { isAuthenticated } = useAuth();
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        // Select the most recent session for the current mode
        if (!activeChatId && sortedSessions.length > 0) {
          const modeSessions = sortedSessions.filter(
            (s) => s.mode === activeMode
          );
          if (modeSessions.length > 0) {
            setActiveChatId(modeSessions[0].id);
          } else {
            setActiveChatId(null);
          }
        }
      ...
      setError(
        err.response?.data?.error ||
          err.message ||
          "Network error occurred while fetching sessions"
      );
    } finally {
      setLoading(false);
    }
  }, [isAuthenticated, activeChatId, activeMode]);
  ...
  const fetchPortalIntro = async (sessionId) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          err.message ||
          "Network error occurred while fetching portal intro"
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchMessages = async (sessionId) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          err.message ||
          "Network error occurred while fetching messages"
      );
    } finally {
      setLoading(false);
    }
  ...
  const createSession = async (mode) => {
    if (!mode) {
      setError("Mode is required to create a session");
    ...
    try {
      const result = await chatService.createSession(mode);
  ...
  const finishSession = async (summary) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.error ||
        err.message ||
        "Error occurred while finishing session";
      setError(errorMsg);
      return { success: false, error: errorMsg };
    } finally {
      setLoading(false);
  ...
  const sendMessage = async (message, mode) => {
    ...
    if (!mode) {
      setError("Mode is required to send a message");
      return { success: false, error: "Mode is required" };
    }

    let currentChatId = activeChatId;

    if (!currentChatId && (mode === "map" || mode === "imaginary")) {
      try {
        const sessionResult = await createSession(mode);
